...
const sizeToFree = neededSize - (deviceSize - rootNode.size);

const parseDirSize = (node: TreeNode) => {
  for (const child of node.children) {
    parseDirSize(child);
  }
  if (node.nodeType === 'dir' && node.size >= sizeToFree) {
    sizesOver100K.push(node.size);
  }
};

parseDirSize(rootNode);

const result = Math.min(...sizesOver100K);
